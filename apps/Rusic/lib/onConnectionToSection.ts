...
export async function onConnectionToSection(sectionprops: string) {
	...
	console.log("--------CUrrently loegged in user is", Clientuser);
	...
	try {
		...
		if (sectioninfo != null && sectioninfo != undefined) {
			...
			exportVariable.createdBy = sectioninfo.createrId;
			if (streamerInformation != undefined) {
				exportVariable.isOwner = (streamerInformation.id == Clientuser.id) ? true : false;
			}
		} else {
